...
export const recordPrediction = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.latencyMs !== undefined && (!Number.isSafeInteger(args.latencyMs) || args.latencyMs < 0)) {
      throw new Error("latency must be a non-negative integer");
    }
    const existing = await ctx.db.query("aiPredictions").withIndex("by_trace", (q) => q.eq("traceId", args.traceId)).unique();
    if (existing) {
      if (existing.responseJson !== args.responseJson || existing.predictionType !== args.predictionType) {
